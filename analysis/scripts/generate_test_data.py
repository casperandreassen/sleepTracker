...
def generate_data(num_days):
    ...
    for i in range(num_days):
        ...
        day.append(("wake", random_iso8601_time(date, wake_up_time[0], wake_up_time[1]).isoformat()))
        for nap in range(naps):
            day.append(("sleep", random_iso8601_time(date, nap_times[nap][0], nap_times[nap][1]).isoformat()))
            day.append(("wake", random_iso8601_time(date, wake_times[nap][0], wake_times[nap][1]).isoformat()))
        day.append(("sleep", random_iso8601_time(date, bedtime[0], bedtime[1]).isoformat()))
